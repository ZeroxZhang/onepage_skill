...
> 💡 基于内容分析，我推荐：**A? B? C? D?**（请告诉我你的选择，或直接采纳推荐）
...
- 全页色彩不超过 3 个主色 + 2 个中性色（B5 孟菲斯可适当放宽数量，但保持比例原则）
- 字体家族不超过 2 种（一种标题，一种正文）
- 图标风格全页统一：线性/面性/拟物三选一
...
**视觉原则**：
5. **简约而不简单**：留白是设计的一部分，但关键位置的渐变/光效/几何元素让画面有呼吸感
6. **图文一体**：图表不是装饰品，它必须与文字内容形成互补关系
7. **中文优先**：所有排版决策以中文阅读体验为第一优先级
8. **像素级执行**：每一个 CSS 属性都要经过思考，不可使用粗糙的默认值