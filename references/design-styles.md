...
全页色彩不超过 **3 个主色 + 2 个中性色**。B5 孟菲斯风格可适当放宽色彩数量，但需保持大致的 60-30-10 比例原则。
...
### 色彩预算映射
- 60% 主色：深色背景 `#0a0a1a`-`#0f0f2e` 渐变
- 30% 辅助色：卡片半透明表面 + 正文白色
- 10% 强调色：青 `#00d4ff` / 紫 `#7b5cff`（仅用于 CTA、关键数据、装饰高光）

### 图标风格推荐
- **线性图标**（精细、科技感，线宽 1.5-2px）
...
### 色彩预算映射
- 60% 主色：白色/浅灰背景 `#ffffff` / `#f8f9fa`
- 30% 辅助色：蓝 `#2563eb` 标题/区块 + 深墨 `#0f172a` 正文
- 10% 强调色：紫 `#7c3aed` 渐变点缀（仅用于小面积装饰、CTA）

### 图标风格推荐
- **线性图标**（专业、克制，线宽 1.5-2px）
...
### 色彩预算映射
- 60% 主色：白色/微绿背景 `#fefffe`-`#f0fdf4`
- 30% 辅助色：绿 `#10b981` / 青 `#06b6d4` 标题/区块 + 弥散光影
- 10% 强调色：紫 `#8b5cf6` 重点标注
...
### 色彩预算映射
- 60% 主色：灰蓝背景 `#e0e5ec`
- 30% 辅助色：靛蓝 `#6366f1` 标题/区块 + 灰蓝 `#475569` 正文
- 10% 强调色：粉红 `#ec4899` 重点标注

### 图标风格推荐
- **面性图标**（有触感、3D 效果，与光影系统一致）
...
### 色彩预算映射
- 60% 主色：白色/浅灰背景 `#ffffff` / `#f5f5f5`（或反转深黑 `#111111`）
- 30% 辅助色：撞色色块（亮绿、明黄、玫红、宝蓝）用于卡片/区块
- 10% 强调色：最高饱和度对比色用于 CTA/关键数据

> **B5 色彩例外**：孟菲斯风格可使用超过 3+2 的色彩数量，但需保持 60% 中性底 / 30% 主题色 / 10% 强调色的大致比例。中性色比例是视觉平衡的锚点。

### 图标风格推荐
- **拟物/插画风格**（大胆、有趣，可使用手绘质感）
...
### 色彩预算映射
- 60% 主色：浅灰微渐变背景 `#f8f9fc`
- 30% 辅助色：深灰 `#2d3748` 正文 + 半透明白卡片/区块
- 10% 强调色：亮紫 `#8a2be2` / 亮绿 `#00fa9a` 仅用于 CTA、关键数据

### 图标风格推荐
- **面性图标**（圆润、轻盈，圆角 16-24px，与软泡形态一致）